...
from relobstq_mhn.core.states import (
    canonical_genotype,
    genotype_events,
    genotype_signature,
)
...
try:
    ...
    MHN_AVAILABLE = False
CONFIG_PATH = bundled_config_path("longitudinal.yaml")
...
def infer_temporal_metadata(
    study_id: str, study_dir: Path, sample_df: pd.DataFrame
) -> pd.DataFrame:
    if sample_df.empty:
        return pd.DataFrame()
    work = sample_df.copy()
    if "SAMPLE_ID" not in work or "PATIENT_ID" not in work:
        raise ValueError(
            f"{study_id}: clinical sample table lacks SAMPLE_ID/PATIENT_ID"
        )
    ...
    if study_id == "difg_glass":
        ...
        work["stage"] = np.where(
            work["time_rank"].fillna(-1).eq(0), "baseline", "progressed"
        )
        ...
        work["time_rank"] = (
            work["sample_id"]
            .map(surgery_times)
            .where(work["sample_id"].map(surgery_times).notna(), work["time_rank"])
        )
        work["order_evaluable"] = work["time_rank"].notna()
    elif study_id == "nsclc_tracerx_2017":
        sample_type = work.get("SAMPLE_TYPE", "").fillna("").astype(str).str.lower()
        timepoint = (
            work.get("SAMPLE_COLLECTION_TIMEPOINT", "")
            .fillna("")
            .astype(str)
            .str.lower()
        )
        progressed = sample_type.str.contains(
            "recurrence", na=False
        ) | timepoint.str.contains("post", na=False)
        baseline = sample_type.str.contains(
            "primary", na=False
        ) | timepoint.str.contains("pre", na=False)
        work["time_rank"] = np.where(progressed, 1.0, np.where(baseline, 0.0, np.nan))
        work["stage"] = np.where(
            progressed, "progressed", np.where(baseline, "baseline", "unknown")
        )
        work["sample_role"] = np.where(
            progressed, "recurrence_or_post_treatment", "primary_or_pre_treatment"
        )
        ...
        suffix = pd.to_numeric(
            work["sample_id"].str.extract("-(\\d+)$", expand=False), errors="coerce"
        )
        work["time_rank"] = suffix
        work["stage"] = np.where(
            suffix.eq(1), "baseline", np.where(suffix.gt(1), "progressed", "unknown")
        )
        work["sample_role"] = np.where(
            suffix.eq(1),
            "paired_early_sample",
        ...
        work["sample_role"] = np.where(
            progressed, "metastasis_or_local_recurrence", "primary"
        )
        work.loc[work["stage"].eq("unknown"), "sample_role"] = "unknown"
        time_lookup = read_msk_chord_specimen_times(study_dir)
        work["time_rank"] = work["sample_id"].map(time_lookup)
        fallback_order = (
            work["sample_id"].str.extract("-T(\\d+)-", expand=False).map(numeric_or_nan)
        )
        work["time_rank"] = work["time_rank"].where(
            work["time_rank"].notna(), fallback_order
        )
        work.loc[work["time_rank"].isna() & baseline, "time_rank"] = 0.0
        work.loc[work["time_rank"].isna() & progressed, "time_rank"] = 1.0
        work["order_evaluable"] = work["time_rank"].notna() & work["stage"].ne(
            "unknown"
        )
        ...
        work["sample_role"] = np.where(
            relapse, "relapse_flag_positive", "relapse_flag_negative"
        )
        ...
        age = (
            work.get("AGE_AT_PROCUREMENT", "").map(numeric_or_nan)
            if "AGE_AT_PROCUREMENT" in work
            else np.nan
        )
        ...
        work["sample_role"] = np.where(
            progressed,
            "metastatic_site",
            np.where(baseline, "prostate_site", "unknown"),
        )
        work["time_rank"] = age
        work["order_evaluable"] = work["time_rank"].notna() & work["stage"].ne(
            "unknown"
        ...
        work["sample_role"] = np.where(
            progressed, "cycle_1_day_3", np.where(baseline, "baseline", "unknown")
        )
        work["order_evaluable"] = work["time_rank"].notna()
    elif study_id == "brca_mbcproject_2022":
        bx_day = work.get("BX_TIME_DAYS", pd.Series(np.nan, index=work.index)).map(
            numeric_or_nan
        )
        fallback_time = (
            work.get("SAMPLE_TIMEPOINT", "")
            .fillna("")
            .astype(str)
            .str.extract("_T(\\d+)", expand=False)
            .map(numeric_or_nan)
        )
        ...
        baseline = setting.str.contains(
            "no_metastatic", na=False
        ) | location.str.contains("breast", na=False)
        progressed = (
            setting.str.contains("metastatic_disease_present", na=False) & ~baseline
        )
        work["stage"] = np.where(
            progressed, "progressed", np.where(baseline, "baseline", "unknown")
        )
        role = np.where(
            progressed,
            "metastatic_or_liquid_biopsy",
            np.where(baseline, "breast_or_non_metastatic", "unknown"),
        )
        ...
        sample_type = (
            work.get("SAMPLE_TYPE", work.get("CBIOPORTAL_SAMPLE_TYPE", ""))
            .fillna("")
            .astype(str)
            .str.lower()
        )
        cbio_type = (
            work.get("CBIOPORTAL_SAMPLE_TYPE", "").fillna("").astype(str).str.lower()
        )
        baseline = sample_type.str.contains(
            "primary", na=False
        ) | cbio_type.str.contains("primary", na=False)
        progressed = sample_type.str.contains(
            "metast", na=False
        ) | cbio_type.str.contains("metast", na=False)
        work["time_rank"] = np.where(progressed, 1.0, np.where(baseline, 0.0, np.nan))
        work["stage"] = np.where(
            progressed, "progressed", np.where(baseline, "baseline", "unknown")
        )
        site = (
            work.get("METASTATIC_SITE", "")
            .fillna("")
            .astype(str)
            .str.lower()
        ...
        work["sample_role"] = np.where(
            progressed, "metastasis_" + site, np.where(baseline, "primary", "unknown")
        )
        work.loc[
            pd.Series(work["sample_role"], index=work.index)
            .astype(str)
            .eq("metastasis_"),
            "sample_role",
        ] = "metastasis"
        ...
        baseline = sample_id.str.contains(
            "(?:^|[_-])pre(?:$|[_-])", regex=True, na=False
        )
        progressed = sample_id.str.contains(
            "(?:^|[_-])post(?:$|[_-])", regex=True, na=False
        )
        work["time_rank"] = np.where(progressed, 1.0, np.where(baseline, 0.0, np.nan))
        work["stage"] = np.where(
            progressed, "progressed", np.where(baseline, "baseline", "unknown")
        ...
        work["sample_role"] = np.where(
            progressed,
            "post_braf_inhibitor",
            np.where(baseline, "pre_braf_inhibitor", "unknown"),
        )
        work["order_evaluable"] = work["time_rank"].notna()
        ...
        baseline = sample_type.str.contains(
            "primary", na=False
        ) & sample_class.str.contains("tumor", na=False)
        progressed = sample_type.str.contains(
            "metast", na=False
        ) & sample_class.str.contains("tumor", na=False)
        work["time_rank"] = np.where(progressed, 1.0, np.where(baseline, 0.0, np.nan))
        ...
        work["sample_role"] = np.where(
            progressed,
            "metastasis",
            np.where(baseline, "primary", "non_tumor_or_unknown"),
        )
        work["order_evaluable"] = work["time_rank"].notna()
        ...
        work["sample_role"] = np.where(
            progressed,
            "lymph_node_or_distant_metastasis",
            np.where(baseline, "primary", "unknown"),
        )
        work["order_evaluable"] = work["time_rank"].notna()
    elif study_id == "bm_nsclc_mskcc_2023":
        specimen = (
            work.get("TISSUE_SPECIMEN_TYPE", "").fillna("").astype(str).str.upper()
        )
        baseline = specimen.eq("PT") | work.get("SAMPLE_TYPE", "").fillna("").astype(
            str
        ).str.lower().str.contains("primary", na=False)
        extracranial_met = specimen.eq("EM")
        brain_met = specimen.eq("BM")
        work["time_rank"] = np.where(
            brain_met,
            2.0,
            np.where(extracranial_met, 1.0, np.where(baseline, 0.0, np.nan)),
        )
        work["stage"] = np.where(
            brain_met | extracranial_met,
            "progressed",
            np.where(baseline, "baseline", "unknown"),
        )
        work["sample_role"] = np.where(
            brain_met,
            "brain_metastasis",
        ...
        site = (
            work.get("SITE_OF_TISSUE_BIOPSY", "")
            .fillna("")
            .astype(str)
            .str.lower()
            .str.replace("\\s+", "_", regex=True)
        ...
        progressed = sample_type.str.contains("metast", na=False)
        work["time_rank"] = np.where(
            progressed, 1.0, np.where(csf, 0.5, np.where(baseline, 0.0, np.nan))
        )
        work["stage"] = np.where(
            progressed | csf, "progressed", np.where(baseline, "baseline", "unknown")
        )
        work["sample_role"] = np.where(
            progressed,
            "metastasis",
            np.where(
                csf, "csf_liquid_biopsy", np.where(baseline, "primary", "unknown")
            ),
        )
        work["order_evaluable"] = work["time_rank"].notna()
    else:
        ...
        progressed = sample_type.str.contains(
            "metast|recurrence|post", regex=True, na=False
        )
        work["stage"] = np.where(
            progressed, "progressed", np.where(baseline, "baseline", "unknown")
        )
...
def iter_mutation_chunks(
    path: Path, sample_ids: set[str], genes: set[str]
    ...
    for chunk in pd.read_csv(
        ...
        chunk = chunk.rename(
            columns={"Hugo_Symbol": "gene", "Tumor_Sample_Barcode": "sample_id"}
        )
        chunk["gene"] = chunk["gene"].map(clean_gene)
        chunk["sample_id"] = chunk["sample_id"].astype(str)
        chunk = chunk[
            chunk["sample_id"].isin(sample_ids) & chunk["gene"].isin(genes)
        ].copy()
        if chunk.empty:
            continue
...
def load_driver_mutations(
    study_id: str, study_dir: Path, metadata: pd.DataFrame
) -> pd.DataFrame:
    ...
    for mutation_path in sorted(study_dir.glob("data_mutations*.txt")):
        for chunk in iter_mutation_chunks(mutation_path, sample_ids, genes):
            pairs.update(
                (
                    (str(row.sample_id), str(row.gene))
...
def select_events(
    study_id: str, metadata: pd.DataFrame, mutations: pd.DataFrame, config: dict
    ...
    if mutations.empty:
        return (
            [],
            pd.DataFrame(columns=["event", "sample_count", "frequency", "selected"]),
        )
    support = (
    ...
    priority = {
        gene: idx for idx, gene in enumerate(STUDY_PRIORITY_GENES.get(study_id, []))
    }
    table["priority_rank"] = table["event"].map(priority).fillna(10000).astype(int)
    ...
    candidates = candidates.sort_values(
        ["sample_count", "priority_rank", "event"], ascending=[False, True, True]
    )
    selected = candidates["event"].head(max_events).tolist()
    table["selected"] = table["event"].isin(selected)
    table = table.sort_values(
        ["selected", "sample_count", "priority_rank"], ascending=[False, False, True]
    )
...
def build_event_matrix(
    ...
    if events and (not mutations.empty):
        work = (
            mutations[mutations["gene"].isin(events)]
        ...
        pivot = (
            pivot.reindex(index=matrix["sample_id"], columns=events, fill_value=0)
            .astype(int)
            .reset_index()
        )
        matrix = pivot
...
def build_occupancy(
    metadata: pd.DataFrame, matrix: pd.DataFrame, events: list[str]
) -> pd.DataFrame:
    merged = metadata.merge(matrix, on="sample_id", how="left")
    for event in events:
        merged[event] = (
            pd.to_numeric(merged[event], errors="coerce").fillna(0).astype(int)
        )
    merged["genotype"] = [
        genotype_signature(row, events) for row in merged[events].to_numpy(dtype=int)
    ]
    ...
    merged["state"] = (
        merged["stage"].astype(str).str.lower() + "::" + merged["genotype"].astype(str)
    )
    occupancy = (
    ...
    return occupancy.sort_values(["N_v", "state"], ascending=[False, True]).reset_index(
        drop=True
    )


def fit_or_build_theta(
    matrix: pd.DataFrame, events: list[str], config: dict, seed: int
                ...
                return (
                    theta,
                    {
                        ...
                        "lambda_multiplier": float(
                            config["mhn"]["fixed_lambda_multiplier"]
                        ),
                        "fit_status": "ok",
                    },
                )
...
def score_external_states(
    occupancy: pd.DataFrame, theta: np.ndarray, events: list[str], config: dict
    ...
    thresholds = ScoreThresholds(
        ...
        high_confidence_state_count=int(
            threshold_values["high_confidence_state_count"]
        ),
    )
    ...
    try:
        ...
        relaxed = ScoreThresholds(
            ...
            minimum_inflow=thresholds.minimum_inflow,
            high_confidence_state_count=max(
                2, thresholds.high_confidence_state_count - 1
            ),
        )
        scores, normalizer = compute_relative_dwell(inflow, relaxed)
    scores = classify_relative_states(scores)
    return (
        scores.sort_values(["R_star", "N_v"], ascending=[False, False]).reset_index(
            drop=True
        ),
        edges,
        normalizer,
...
def aggregate_timepoint_events(
    metadata: pd.DataFrame, matrix: pd.DataFrame, events: list[str]
) -> pd.DataFrame:
    merged = metadata.merge(matrix, on="sample_id", how="left")
    for event in events:
    ...
    usable = merged[
        merged["order_evaluable"].astype(bool) & merged["time_rank"].notna()
    ].copy()
    if usable.empty:
        return pd.DataFrame()
    rows = []
    for (patient_id, time_rank), group in usable.groupby(
        ["patient_id", "time_rank"], dropna=True
    ):
        stage = "progressed" if group["stage"].eq("progressed").any() else "baseline"
        event_values = group[events].max(axis=0).astype(int).to_dict()
        genotype = genotype_signature(
            np.array([event_values[event] for event in events], dtype=int), events
        )
        rows.append(
            {
    ...
    return (
        pd.DataFrame(rows)
        .sort_values(["patient_id", "time_rank"])
...
def bootstrap_ci(
    evaluable: pd.DataFrame, metric_fn, seed: int, n_bootstrap: int
) -> tuple[float, float]:
    ...
    if len(patient_ids) >= 2:
        patient_groups = {
            patient: group
            for patient, group in evaluable.groupby(evaluable["patient_id"].astype(str))
        }
        for _ in range(n_bootstrap):
            sampled_patients = rng.choice(
                patient_ids, size=len(patient_ids), replace=True
            )
            sample = pd.concat(
                [patient_groups[patient] for patient in sampled_patients],
                ignore_index=True,
            )
...
def dwell_contrast(
    ...
    score_lookup = scores.set_index("state")[
        ["R_star", "log2_R_star", "eligible_relobstq"]
    ...
    merged["genotype"] = merged["genotype"].map(canonical_genotype)
    merged["state"] = (
        merged["stage"].astype(str).str.lower() + "::" + merged["genotype"]
    )
    merged["R_star"] = merged["state"].map(
        lambda state: score_lookup.get(state, {}).get("R_star", np.nan)
    )
    merged["log2_R_star"] = merged["state"].map(
        lambda state: score_lookup.get(state, {}).get("log2_R_star", np.nan)
    )
    merged["eligible_relobstq"] = merged["state"].map(
        lambda state: score_lookup.get(state, {}).get("eligible_relobstq", False)
    )
    eligible = merged[
        merged["eligible_relobstq"].astype(bool)
        & merged["stage"].isin(["baseline", "progressed"])
    ].copy()
    rows = []
    baseline = eligible.loc[eligible["stage"].eq("baseline"), "log2_R_star"].dropna()
    progressed = eligible.loc[
        eligible["stage"].eq("progressed"), "log2_R_star"
    ].dropna()
    if len(baseline) and len(progressed):
        try:
            p_value = float(
                mannwhitneyu(progressed, baseline, alternative="two-sided").pvalue
            )
        except Exception:
            p_value = np.nan
        rows.append(
            {
                ...
                "progressed_minus_baseline_median_log2_R": float(
                    np.median(progressed) - np.median(baseline)
                ),
...
def build_longitudinal_pair_table(
    study_id: str, timepoints: pd.DataFrame, events: list[str]
) -> pd.DataFrame:
            ...
            rows.append(
                {
                    ...
                    "time_interval_rank": float(later["time_rank"])
                    - float(earlier["time_rank"]),
...
def weighted_genotype_log2_r(scores: pd.DataFrame) -> dict[str, float]:
    ...
    for genotype, group in eligible.groupby("genotype_key"):
        values = pd.to_numeric(group["log2_R_star"], errors="coerce").replace(
            [np.inf, -np.inf], np.nan
        )
        weights = (
            pd.to_numeric(group["N_v"], errors="coerce").fillna(1.0).clip(lower=1.0)
        )
        valid = values.notna()
        if valid.any():
            result[str(genotype)] = float(
...
def score_from_training_patients(
    ...
    train_metadata = metadata[
        metadata["patient_id"].astype(str).isin(train_patients)
    ].copy()
    ...
    threshold = float(
        np.nanmedian(eligible["log2_R_star"].replace([np.inf, -np.inf], np.nan))
    )
...
def evaluate_rstar_dwell_persistence(
    ...
    fold_lookup = {
        patient: index % n_folds for index, patient in enumerate(pair_patients)
    }
    all_patients = set(metadata["patient_id"].astype(str).unique())
    prediction_rows = []
    for fold in range(n_folds):
        validation_patients = {
            patient for patient, fold_index in fold_lookup.items() if fold_index == fold
        }
        train_patients = all_patients.difference(validation_patients)
        train_scores, threshold = score_from_training_patients(
            metadata, matrix, events, theta, train_patients, config
        )
        ...
        validation_pairs = pairs[
            pairs["patient_id"].astype(str).isin(validation_patients)
        ].copy()
    ...
    evaluable = filtered_predictions[
        np.isfinite(
            pd.to_numeric(filtered_predictions["predicted_log2_R"], errors="coerce")
        )
    ].copy()
    ...
    auc = (
        binary_auc(labels, score_values)
    ...
    predictions.loc[evaluable.index, "rstar_contrast_group"] = evaluable[
        "rstar_contrast_group"
    ]
    ...
    delta_rate = (
        high_rate - low_rate
        if np.isfinite(high_rate) and np.isfinite(low_rate)
        else np.nan
    )
    ...
    average_precision_lift = (
        ...
        if np.isfinite(average_precision_value)
    ...
    kendall_state_tau, kendall_state_p = safe_rank_correlation(
        score_values, jaccard_values, method="kendall"
    )
    high_persistent = (
        int(high["empirical_persistent"].astype(int).sum()) if not high.empty else 0
    )
    high_changed = int(len(high) - high_persistent)
    low_persistent = (
        int(low["empirical_persistent"].astype(int).sum()) if not low.empty else 0
    )
    ...
    bottom_rstar_lift_over_base = (
        low_rate / base_rate
        if np.isfinite(low_rate) and np.isfinite(base_rate) and (base_rate > 0)
        else np.nan
    )
    persistent_scores = evaluable.loc[
        evaluable["empirical_persistent"].astype(int).eq(1), "predicted_log2_R"
    ]
    changed_scores = evaluable.loc[
        evaluable["empirical_persistent"].astype(int).eq(0), "predicted_log2_R"
    ]
    if len(persistent_scores) and len(changed_scores):
        mw_p = float(
            mannwhitneyu(
                persistent_scores, changed_scores, alternative="greater"
            ).pvalue
        )
    else:
        mw_p = np.nan
    ...
    interval_values = np.log1p(
        interval_valid["time_interval_rank"].astype(float).to_numpy()
    )
    if (
        len(interval_valid) >= 3
        and np.unique(interval_scores).size > 1
        and (np.unique(interval_values).size > 1)
    ):
        try:
            interval_rho, interval_p = spearmanr(
                interval_scores, interval_values, nan_policy="omit"
            )
        except Exception:
    ...
    median_high_interval = (
        float(np.median(high_interval)) if len(high_interval) else np.nan
    )
    median_low_interval = (
        float(np.median(low_interval)) if len(low_interval) else np.nan
    )
    delta_interval = (
    ...
    if len(high_interval) and len(low_interval):
        interval_mw_p = float(
            mannwhitneyu(high_interval, low_interval, alternative="greater").pvalue
        )
    else:
        interval_mw_p = np.nan
    minimum_dwell_valid = evaluable[
        np.isfinite(
            pd.to_numeric(evaluable["minimum_observed_dwell_interval"], errors="coerce")
        )
    ].copy()
    minimum_dwell_scores = (
        minimum_dwell_valid["predicted_log2_R"].astype(float).to_numpy()
    )
    ...
    if (
        ...
        try:
            minimum_dwell_rho, minimum_dwell_p = spearmanr(
                minimum_dwell_scores, minimum_dwell_values, nan_policy="omit"
            )
        except Exception:
    ...
    minimum_dwell_tau, minimum_dwell_tau_p = safe_rank_correlation(
        minimum_dwell_scores, minimum_dwell_values, method="kendall"
    )
    high_minimum_dwell = high[
        np.isfinite(
            pd.to_numeric(high["minimum_observed_dwell_interval"], errors="coerce")
        )
    ]["minimum_observed_dwell_interval"].astype(float)
    low_minimum_dwell = low[
        np.isfinite(
            pd.to_numeric(low["minimum_observed_dwell_interval"], errors="coerce")
        )
    ]["minimum_observed_dwell_interval"].astype(float)
    mean_high_minimum_dwell = (
        float(np.mean(high_minimum_dwell)) if len(high_minimum_dwell) else np.nan
    )
    mean_low_minimum_dwell = (
        float(np.mean(low_minimum_dwell)) if len(low_minimum_dwell) else np.nan
    )
    delta_minimum_dwell = (
    ...
    if len(high_minimum_dwell) and len(low_minimum_dwell):
        minimum_dwell_mw_p = float(
            mannwhitneyu(
                high_minimum_dwell, low_minimum_dwell, alternative="greater"
            ).pvalue
        )
    else:
    ...
    bootstrap_seed = (
        ...
        + int(hashlib.sha256(study_id.encode("utf-8")).hexdigest()[:8], 16) % 10000
    )
    ...
    def _delta_metric(frame: pd.DataFrame) -> float:
        ...
        if (
            np.isfinite(local_low_cut)
            and np.isfinite(local_high_cut)
            and (local_low_cut < local_high_cut)
        ):
        ...
        if local_low.empty or local_high.empty:
            return np.nan
        return float(
            local_high["empirical_persistent"].mean()
            - local_low["empirical_persistent"].mean()
        )
    ...
    auc_ci_low, auc_ci_high = bootstrap_ci(
        evaluable, _auc_metric, bootstrap_seed + 1, n_bootstrap
    )
    ap_ci_low, ap_ci_high = bootstrap_ci(
        evaluable, _ap_metric, bootstrap_seed + 2, n_bootstrap
    )
    delta_ci_low, delta_ci_high = bootstrap_ci(
        evaluable, _delta_metric, bootstrap_seed + 3, n_bootstrap
    )
    dwell_rho_ci_low, dwell_rho_ci_high = bootstrap_ci(
        evaluable, _minimum_dwell_rho_metric, bootstrap_seed + 4, n_bootstrap
    )
    summary = pd.DataFrame(
        [
            {
                ...
                **pair_qc,
                ...
                "spearman_r_interval": (
                    float(interval_rho) if np.isfinite(interval_rho) else np.nan
                ),
                "spearman_p_interval": (
                    float(interval_p) if np.isfinite(interval_p) else np.nan
                ),
                "spearman_r_minimum_dwell_interval": (
                    float(minimum_dwell_rho)
                    if np.isfinite(minimum_dwell_rho)
                    else np.nan
                ),
                ...
                "median_log2_R_persistent": (
                    float(np.median(persistent_scores))
                    if len(persistent_scores)
                    else np.nan
                ),
                "median_log2_R_changed": (
                    float(np.median(changed_scores)) if len(changed_scores) else np.nan
                ),
                "mannwhitney_p": mw_p,
                "exact_state_score_fraction": float(
                    evaluable["score_source"].eq("exact_state").mean()
                ),
            }
        ]
    )
...
def make_metric_audit(dwell_persistence: pd.DataFrame) -> pd.DataFrame:
    ...
    for item in definitions:
        ...
        if metric in dwell_persistence.columns:
            values = (
                pd.to_numeric(dwell_persistence[metric], errors="coerce")
        ...
        if item["direction"] == "higher_than_0.5":
            ...
            supportive_count = 0
            if (
                metric in dwell_persistence.columns
                and "persistence_rate" in dwell_persistence.columns
            ):
                for _, row in dwell_persistence.iterrows():
                    ...
                    base = pd.to_numeric(
                        pd.Series([row.get("persistence_rate")]), errors="coerce"
                    ).iloc[0]
...
def make_core_metric_table(
    dwell_persistence: pd.DataFrame, cohort_qc: pd.DataFrame
) -> pd.DataFrame:
    ...
    for row in dwell_persistence.to_dict(orient="records"):
        ...
        short_name = (
            qc_lookup.loc[study_id, "short_name"]
            if study_id in qc_lookup.index
            else study_id
        )
        auc_text = format_float(row.get("auc"), 2)
        if np.isfinite(
            pd.to_numeric(pd.Series([row.get("auc_ci_low")]), errors="coerce").iloc[0]
        ) and np.isfinite(
            pd.to_numeric(pd.Series([row.get("auc_ci_high")]), errors="coerce").iloc[0]
        ):
            auc_text = f"{auc_text} [{format_float(row.get('auc_ci_low'), 2)}, {format_float(row.get('auc_ci_high'), 2)}]"
        delta_text = format_float(row.get("delta_persistence_rate_high_minus_low"), 2)
        if np.isfinite(
            pd.to_numeric(
                pd.Series([row.get("delta_persistence_ci_low")]), errors="coerce"
            ).iloc[0]
        ) and np.isfinite(
            pd.to_numeric(
                pd.Series([row.get("delta_persistence_ci_high")]), errors="coerce"
            ).iloc[0]
        ):
            delta_text = f"{delta_text} [{format_float(row.get('delta_persistence_ci_low'), 2)}, {format_float(row.get('delta_persistence_ci_high'), 2)}]"
        rho_text = format_float(row.get("spearman_r_minimum_dwell_interval"), 2)
        if np.isfinite(
            pd.to_numeric(
                pd.Series([row.get("spearman_r_minimum_dwell_ci_low")]), errors="coerce"
            ).iloc[0]
        ) and np.isfinite(
            pd.to_numeric(
                pd.Series([row.get("spearman_r_minimum_dwell_ci_high")]),
                errors="coerce",
            ).iloc[0]
        ):
            rho_text = f"{rho_text} [{format_float(row.get('spearman_r_minimum_dwell_ci_low'), 2)}, {format_float(row.get('spearman_r_minimum_dwell_ci_high'), 2)}]"
        rows.append(
            {
                "cohort": short_name,
                ...
                "exact_state_fraction": format_float(
                    row.get("exact_state_score_fraction"), 2
                ),
            }
        )
    return pd.DataFrame(rows)
...
def process_study(
    study_id: str, study_config: dict, config: dict, result_root: Path
) -> dict:
    ...
    theta, fit_meta = fit_or_build_theta(
        matrix, events, local_config, validation_seed + study_seed
    )
    scores, edges, normalizer = score_external_states(
        occupancy, theta, events, local_config
    )
    ...
    dwell_predictions, dwell_persistence = evaluate_rstar_dwell_persistence(
        study_id, metadata, matrix, timepoints, theta, events, local_config
    )
    dwell, paired_dwell = dwell_contrast(
        study_id, metadata, matrix, scores, timepoints, events
    )
    ...
    pd.DataFrame(theta, index=events, columns=events).rename_axis(
        "target_event"
    ).to_csv(tables_dir / "theta.tsv", sep="\t")
    scores.to_csv(tables_dir / "state_scores.tsv", sep="\t", index=False)
    edges.to_csv(tables_dir / "state_edges.tsv", sep="\t", index=False)
    timepoints.to_csv(
        tables_dir / "ordered_timepoint_states.tsv", sep="\t", index=False
    )
    dwell_predictions.to_csv(
        tables_dir / "dwell_persistence_predictions.tsv", sep="\t", index=False
    )
    dwell_persistence.to_csv(
        tables_dir / "dwell_persistence_summary.tsv", sep="\t", index=False
    )
    ...
    (tables_dir / "fit_metadata.json").write_text(
        json.dumps(fit_meta, indent=2), encoding="utf-8"
    )
    multi_sample = metadata.groupby("patient_id")["sample_id"].nunique()
    ordered_pairs = (
        int(dwell_persistence["evaluable_pairs"].iloc[0])
        if not dwell_persistence.empty
        else 0
    )
    raw_ordered_pairs = (
        int(dwell_persistence["total_ordered_pairs"].iloc[0])
        if not dwell_persistence.empty
        else 0
    )
    retained_pair_qc = (
        int(dwell_persistence["pair_qc_retained_pairs"].iloc[0])
        if not dwell_persistence.empty
        else 0
    )
    excluded_loss_pairs = (
        int(dwell_persistence["pair_qc_excluded_event_loss_pairs"].iloc[0])
        if not dwell_persistence.empty
        else 0
    )
    eligible_scores = scores[scores["eligible_relobstq"].astype(bool)].copy()
    qc = {
        ...
        "eligible_states": (
            int(scores["eligible_relobstq"].astype(bool).sum())
            if "eligible_relobstq" in scores
            else 0
        ),
        "top_R_star": (
            float(eligible_scores["R_star"].replace([np.inf, -np.inf], np.nan).max())
            if not eligible_scores.empty
            else np.nan
        ),
        "median_R_star": (
            float(eligible_scores["R_star"].median())
            if not eligible_scores.empty
            else np.nan
        ),
    ...
    return {
        ...
        "scores": scores.assign(
            study_id=study_id, short_name=study_config["short_name"]
        ),
        "dwell": dwell,
...
def main() -> None:
    ...
    scores_all = pd.concat(
        [value["scores"] for value in outputs.values()], ignore_index=True
    )
    scores_all.to_csv(
        result_root / "tables" / "state_scores_all.tsv", sep="\t", index=False
    )
    top_states = (
    ...
    top_states.to_csv(
        result_root / "tables" / "top_external_rstar_states.tsv", sep="\t", index=False
    )
    dwell = pd.concat([value["dwell"] for value in outputs.values()], ignore_index=True)
    paired_dwell_frames = [
        value["paired_dwell"]
        for value in outputs.values()
        if not value["paired_dwell"].empty
    ]
    paired_dwell = (
        pd.concat(paired_dwell_frames, ignore_index=True)
        if paired_dwell_frames
        else pd.DataFrame()
    )
    dwell_prediction_frames = [
        value["dwell_predictions"]
        for value in outputs.values()
        if not value["dwell_predictions"].empty
    ]
    dwell_predictions = (
        pd.concat(dwell_prediction_frames, ignore_index=True)
        if dwell_prediction_frames
        else pd.DataFrame()
    )
    dwell_persistence = pd.concat(
        [value["dwell_persistence"] for value in outputs.values()], ignore_index=True
    )
    dwell.to_csv(
        result_root / "tables" / "dwell_stage_contrast_all.tsv", sep="\t", index=False
    )
    paired_dwell.to_csv(
        result_root / "tables" / "paired_dwell_delta_all.tsv", sep="\t", index=False
    )
    dwell_predictions.to_csv(
        result_root / "tables" / "dwell_persistence_predictions_all.tsv",
        sep="\t",
        index=False,
    )
    dwell_persistence.to_csv(
        result_root / "tables" / "dwell_persistence_summary_all.tsv",
        sep="\t",
        index=False,
    )
    core_metric_table = make_core_metric_table(dwell_persistence, cohort_qc)
    core_metric_table.to_csv(
        result_root / "tables" / "core_metric_table.tsv", sep="\t", index=False
    )
    metric_audit_all = make_metric_audit(dwell_persistence)
    metric_audit_all.to_csv(
        result_root / "tables" / "metric_audit_all_integrated.tsv",
        sep="\t",
        index=False,
    )
    primary_studies = set(
        config["analysis"].get("primary_validation_studies", config["studies"].keys())
    )
    primary_dwell_persistence = dwell_persistence[
        dwell_persistence["study_id"].isin(primary_studies)
    ].copy()
    metric_audit = make_metric_audit(primary_dwell_persistence)
    metric_audit.to_csv(
        result_root / "tables" / "metric_audit.tsv", sep="\t", index=False
    )
    for obsolete in [
