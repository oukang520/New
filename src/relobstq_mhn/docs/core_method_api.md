...
This document describes the experiment-independent method package in the
standalone `relobstq_mhn/` folder. The original `run_experiment_*.py` scripts
remain the audited reproduction path for Experiments 1-16, while this folder is
the clean method layer intended for SCI submission, reuse and future refactors.
